...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert layer in self.LAYERS
        # =======OPTIMIZATION: SKIP OPENCLIP PRETRAINED DOWNLOAD=======
        # When running in daemon mode, weights are loaded from safetensors checkpoint
        # later, so we skip the expensive pretrained download from HuggingFace here.
        _skip = os.environ.get("WMA_SKIP_CLIP_PRETRAINED") == "1"
        model, _, _ = open_clip.create_model_and_transforms(
            arch, device=torch.device('cpu'), pretrained=None if _skip else version)
        del model.visual
        self.model = model
...
class FrozenOpenCLIPImageEmbedder(AbstractEncoder):
    ...
    def __init__(self,
        ...
        super().__init__()
        # =======OPTIMIZATION: SKIP OPENCLIP PRETRAINED DOWNLOAD=======
        # Same as FrozenOpenCLIPEmbedder: skip download when daemon loads from checkpoint.
        _skip = os.environ.get("WMA_SKIP_CLIP_PRETRAINED") == "1"
        model, _, _ = open_clip.create_model_and_transforms(
            arch,
            device=torch.device('cpu'),
            pretrained=None if _skip else version,
        )
