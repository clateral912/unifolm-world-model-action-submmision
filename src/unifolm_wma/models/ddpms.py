...
class LatentDiffusion(DDPM):
    ...
    def decode_first_stage(self, z: Tensor, **kwargs: Any) -> Tensor:
        ...
        if z.dtype != vae_dtype:
            z = z.to(vae_dtype)
        results = self.decode_core(z, **kwargs)
        return results.to(orig_dtype)
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def __init__(self,
        ...
        if dp_use_ema and os.environ.get("WMA_INFERENCE") != "1":
        ...
        # ======= OPTIMIZATION No.5 : Cross-Attention KV Cache =======
        self._use_kv_cache = False

    def enable_kv_cache(self):
        """Enable cross-attention KV cache on the inner WMAModel."""
        self.model.diffusion_model.enable_kv_cache()

    def disable_kv_cache(self):
        """Disable cross-attention KV cache and free cached tensors."""
        self.model.diffusion_model.disable_kv_cache()
    # ======= END OPTIMIZATION No.5 =======
